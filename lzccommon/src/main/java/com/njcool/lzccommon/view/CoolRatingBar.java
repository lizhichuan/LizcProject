...
/**
 * Created by chuan on 2017/8/14.
 */

public class CoolRatingBar extends View {
    ...
    public CoolRatingBar(Context context) {
        super(context);
        initView();
    }

    public CoolRatingBar(Context context, AttributeSet attrs) {
        super(context, attrs);
        parseAttrs(attrs);
        initView();
    }

    public CoolRatingBar(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    private void parseAttrs(AttributeSet attrs) {
        TypedArray arr = getContext().obtainStyledAttributes(attrs, R.styleable.CoolRatingBar);

        borderColor = arr.getColor(R.styleable.CoolRatingBar_srb_borderColor, getResources().getColor(R.color.orange));
        fillColor = arr.getColor(R.styleable.CoolRatingBar_srb_fillColor, borderColor);
        starBackgroundColor = arr.getColor(R.styleable.CoolRatingBar_srb_starBackgroundColor, Color.TRANSPARENT);
        backgroundColor = arr.getColor(R.styleable.CoolRatingBar_srb_backgroundColor, Color.TRANSPARENT);

        pressedBorderColor = arr.getColor(R.styleable.CoolRatingBar_srb_pressedBorderColor, borderColor);
        pressedFillColor = arr.getColor(R.styleable.CoolRatingBar_srb_pressedFillColor, fillColor);
        pressedStarBackgroundColor = arr.getColor(R.styleable.CoolRatingBar_srb_pressedStarBackgroundColor, starBackgroundColor);
        pressedBackgroundColor = arr.getColor(R.styleable.CoolRatingBar_srb_pressedBackgroundColor, backgroundColor);

        numberOfStars = arr.getInteger(R.styleable.CoolRatingBar_srb_numberOfStars, 5);

        starsSeparation = arr.getDimensionPixelSize(R.styleable.CoolRatingBar_srb_starsSeparation, (int) valueToPixels(4, Dimension.DP));
        maxStarSize = arr.getDimensionPixelSize(R.styleable.CoolRatingBar_srb_maxStarSize, Integer.MAX_VALUE);
        desiredStarSize = arr.getDimensionPixelSize(R.styleable.CoolRatingBar_srb_starSize, Integer.MAX_VALUE);
        stepSize = arr.getFloat(R.styleable.CoolRatingBar_srb_stepSize, 0.1f);
        starBorderWidth = arr.getFloat(R.styleable.CoolRatingBar_srb_starBorderWidth, 5f);
        starCornerRadius = arr.getFloat(R.styleable.CoolRatingBar_srb_starCornerRadius, 6f);

        rating = normalizeRating(arr.getFloat(R.styleable.CoolRatingBar_srb_rating, 0f));
        isIndicator = arr.getBoolean(R.styleable.CoolRatingBar_srb_isIndicator, false);
        drawBorderEnabled = arr.getBoolean(R.styleable.CoolRatingBar_srb_drawBorderEnabled, true);
        gravity = Gravity.fromId(arr.getInt(R.styleable.CoolRatingBar_srb_gravity, Gravity.Left.id));
  ...
  /* ----------- GETTERS AND SETTERS ----------- */
    ...
    private void animateRating(AnimationBuilder builder) {
        ...
        ratingAnimator.addListener(new Animator.AnimatorListener() {
            ...
            public void onAnimationEnd(Animator animator) {
                if (ratingListener != null) {
                    ratingListener.onRatingChanged(CoolRatingBar.this, rating, false);
                }
            }
    ...
    public interface OnRatingBarChangeListener {
         ...
         */
        void onRatingChanged(CoolRatingBar simpleRatingBar, float rating, boolean fromUser);
    ...
    public class AnimationBuilder {
        ...
        private AnimationBuilder(CoolRatingBar ratingBar) {
